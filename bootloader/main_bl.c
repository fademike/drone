...
char rx_uart_it=0;   // for rx uart data by uart irq
...
#define BOOTLOADER_KEY_START_ADDRESS                             (uint32_t)0x08003400
#define BOOTLOADER_KEY_VALUE                                     0xAAAA5555
// Flash configuration
#define MAIN_PROGRAM_START_ADDRESS                               (uint32_t)0x08003800
//#define FLASH_PAGE_SIZE                                          1024

int program_state = 0;

uint8_t flash_buf[FLASH_PAGE_SIZE];	// buffer for write flash memory
...
int flash_data_finish (void){ // to load the latest data
	...
	writeFLASH(MAIN_PROGRAM_START_ADDRESS + addr, (int *)flash_buf);
	return 0;
}
int flash_data (uint8_t data){
	flash_buf[flash_data_cnt++] = data;
	if (flash_data_cnt >= FLASH_PAGE_SIZE) {
		writeFLASH(MAIN_PROGRAM_START_ADDRESS + addr, (int *)flash_buf); 	// write buf page to flash
...
int read_pack(uint8_t * pack){
  ...
  // HAL_UART_Transmit(&huart2, pack, sizeof(rf_program_answer), 500);
  HAL_UART_Transmit_DMA(&huart2, pack, sizeof(rf_program_answer));
...
int read_raw_byte(uint8_t data){	// parse rx raw bytes
  ...
  // I had problems with optimization. This helped me. // FIXME

	// else if ((pos == 3)) pos++;						//crc2
	// else if ((pos == 4) && (data == '#')) {read_pack(rx_pack); pos=0; clen=0;}
	else if ((pos == 3)) {read_pack(rx_pack); pos=0; clen=0;}
	else pos = 0;
...
// __disable_irq();
	...
	result = HAL_FLASH_Unlock();
	if (result != HAL_OK) {goto exit_writeflash;}
...
void Write_Bootloader_Key(uint32_t key){
	...
	Printf(" KEY\n\r");
	*(uint32_t *)flash_buf = key;
	writeFLASH(BOOTLOADER_KEY_START_ADDRESS, (int *)flash_buf);
}
uint32_t Read_Bootloader_Key(void){
	readFLASH(BOOTLOADER_KEY_START_ADDRESS, (int *)flash_buf);
	return *(uint32_t * )flash_buf;
}

// #define ARRAY_CB_LEN (1024*2)
// int array_cb[ARRAY_CB_LEN];
// int array_cb_ptr_wr = 0;
// int array_cb_ptr_rd = 0;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
  ...
  // only for uart2:
  read_raw_byte(rx_uart_it);
  // array_cb[array_cb_ptr_wr] = rx_uart_it;
  // array_cb_ptr_wr = (++array_cb_ptr_wr>=ARRAY_CB_LEN) ? 0 : array_cb_ptr_wr;
  
  HAL_UART_Receive_IT(huart, &rx_uart_it, 1);  // Enable USART receive interrupt again
}
...
int main(void)
{
  ...
  if (MC_init < 0) {
    HAL_UART_Receive_IT(&huart2, &rx_uart_it, 1);  // enable external rf via uart
	  Printf("MC FAIL\n\r");
  ...
  if (MC_init >= 0){
    ...
    // if (array_cb_ptr_rd != array_cb_ptr_wr) {
    //   int to_read = array_cb_ptr_wr - array_cb_ptr_rd;
    //   if (to_read < 0) to_read = array_cb_ptr_rd - ARRAY_CB_LEN;

    //   read_raw_buffer(&array_cb[array_cb_ptr_rd], to_read);

    //   array_cb_ptr_rd += to_read;
    //   if (array_cb_ptr_rd >= ARRAY_CB_LEN) array_cb_ptr_rd = 0;
    ...
    if ((bootloader_timeout - l_timer) < 1000) continue; 	// every 1s try start...

    l_timer = bootloader_timeout;

    static int timeout = 0;
    const int ps = 0;
    if (program_state != ps) {	// if programming is running
      program_state = ps;
      timeout = 0;
      continue;
    }
    else { // if programming is stopped, will try start after 2s
      MX_USART2_UART_Init();
      HAL_UART_Receive_IT(&huart2, &rx_uart_it, 1);
      if (timeout++ >= 2) {HAL_Delay(5000);} // wait wdt...
    }
    ...
    //if (MC_init < 0) HAL_Delay(5000);	// wait wdg... because no main program, no init rf. 
