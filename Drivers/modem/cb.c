...
// buff len: char buff[(bsize*sizeof(uint16_t))+(bsize * psize)]
int32_t cb_init_static(circular_buffer * cb, uint16_t bsize, uint16_t psize, char * buff){
    ...
    cb->plen = (uint16_t *) buff;  //(uint16_t *)malloc(bsize * sizeof(uint16_t));
    cb->buf = (uint16_t *) &buff[bsize * sizeof(uint16_t)];  //malloc(bsize * psize);
