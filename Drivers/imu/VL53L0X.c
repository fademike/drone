...
#define VL53L0X_I2C_Handler hi2cN

extern I2C_HandleTypeDef VL53L0X_I2C_Handler; // I2C handler
uint8_t msgBuffer[4];
...
//---------------------------------------------------------
int getSpadInfo(uint8_t *count, bool *type_is_aperture);
...
int vl53_getStatus(void){return statusVL53L0X;}
//---------------------------------------------------------
// I2C communication Functions
//---------------------------------------------------------
// Write an 8-bit register
#define writeReg(x,y) if (writeRegio(x,y) != HAL_OK) {statusVL53L0X = -1; return statusVL53L0X;}
int writeRegio(uint8_t reg, uint8_t value) {
...
int setAddress_VL53L0X(uint8_t new_addr) {
...
bool initVL53L0X(bool io_2v8){
  // VL53L0X_DataInit() begin
	...
	if (statusVL53L0X>0){
		...
		else if (statusVL53L0X==(7)) goto init_pos_4;
		else if (statusVL53L0X==(9)) goto init_pos_5;
	}
  ...
  init_pos_1:
  if (getSpadInfo(&spad_count, &spad_type_is_aperture)!=true) { return false; }
  ...
  // VL53L0X_PerformRefCalibration() end
	...
	int ret1 = setSignalRateLimit(20);
	if (ret1 != true) return -2;
	init_pos_4:
	if (!setVcselPulsePeriod(VcselPeriodPreRange, 14)) { return false; }
//	int ret2 = setVcselPulsePeriod(VcselPeriodPreRange, 14);
//	if (ret2 != true) return -3;
	init_pos_5:
//	int ret3 = setVcselPulsePeriod(VcselPeriodFinalRange, 14);
//	if (ret3 != true) return -4;
	if (!setVcselPulsePeriod(VcselPeriodFinalRange, 14)) { return false; }
	//int ret4 = setMeasurementTimingBudget(300 * 1000UL);
	int ret4 = setMeasurementTimingBudget(30 * 1000UL);
	if (ret4 != true) return -5;

	startContinuous(0);	//FIXME
...
bool setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks)
{
  ...
  if ((statusVL53L0X & 0x1) == 1) goto pulse_pos;
  ...
  static uint8_t sequence_config=0;
  sequence_config = readReg(SYSTEM_SEQUENCE_CONFIG);
  writeReg(SYSTEM_SEQUENCE_CONFIG, 0x02);
  pulse_pos:
  if (performSingleRefCalibration(0x0) == false) return false;
...
int startContinuous(uint32_t period_ms)
{
  writeReg(0x80, 0x01);
  writeReg(0xFF, 0x01);
  writeReg(0x00, 0x00);
...
// extraStats provides additional info for this measurment. Set to 0 if not needed.
int readRangeSingleMillimeters( statInfo_t_VL53L0X *extraStats , uint16_t * data) {
	static int pos = 0;
	if (pos == 0){
	  writeReg(0x80, 0x01);
	  writeReg(0xFF, 0x01);
...
//		}
	  ...
	  readRangeContinuousMillimeters( extraStats, data);
	  pos = 0;
	  return 1;
	}
	return 0;
...
{
  uint8_t tmp;
  if (statusVL53L0X == 1) goto init_info_pos1;
  writeReg(0x80, 0x01);
  writeReg(0xFF, 0x01);
  writeReg(0x00, 0x00);
...
bool performSingleRefCalibration(uint8_t vhv_init_byte)
{
//	if ((statusVL53L0X == 2) || (statusVL53L0X == 4) ||
//			(statusVL53L0X == 6) || (statusVL53L0X == 8)) goto calib_pos;
  if ((statusVL53L0X & 0x1) == 1) goto calib_pos;
  writeReg(SYSRANGE_START, 0x01 | vhv_init_byte); // VL53L0X_REG_SYSRANGE_MODE_START_STOP

  startTimeout();
  //if ((statusVL53L0X>0) || (statusVL53L0X&0x1))
  statusVL53L0X++;
  calib_pos:
  while ((readReg(RESULT_INTERRUPT_STATUS) & 0x07) == 0)
  {
