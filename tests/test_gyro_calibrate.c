

// Используя Unity Test Framework для embedded
#include <stdlib.h>
#include "unity.h"
#include "vector_math.h"
#include "gyro_calibration.h"

void setUp(void) {
    // Сброс состояния перед каждым тестом
    gyro_calibration_init();
}

void tearDown(void) {
    // Очистка после теста
}


vec3_t injected_bias = {0,0,0};
float noise_level = 0.0f;
float move = 0.1f;

void test_calibration_bias_with_noise(void) {

    for (int i = 0; i < CALIB_DURATION_CNT; i++) {
        vec3_t noise;
        // Добавляем шум
        noise.x = (float)(rand() % 100) / (100.0f/noise_level);
        noise.y = (float)(rand() % 100) / (100.0f/noise_level);
        noise.z = (float)(rand() % 100) / (100.0f/noise_level);

        gyro_calibration_update(vec3_add(injected_bias, noise), false);
    }
    
    vec3_t gyro_bias = gyro_calibration_get_bias();

    TEST_ASSERT_FLOAT_WITHIN(noise_level, injected_bias.x, gyro_bias.x);
    TEST_ASSERT_FLOAT_WITHIN(noise_level, injected_bias.y, gyro_bias.y);
    TEST_ASSERT_FLOAT_WITHIN(noise_level, injected_bias.z, gyro_bias.z);

}

// void test_calibration_bias_make_move(void) {

//     float s_noise = move;
//     for (int i = 0; i < CALIB_DURATION_CNT; i++) {
//         vec3_t noise;
//         // Добавляем шум
//         noise.x = (float)(rand() % 100) / (100.0f/noise_level)+s_noise;
//         noise.y = (float)(rand() % 100) / (100.0f/noise_level)+s_noise;
//         noise.z = (float)(rand() % 100) / (100.0f/noise_level)+s_noise;

//         s_noise += move;

//         gyro_calibration_update(vec3_add(injected_bias, noise), false);
//     }
// }

void test_calibration_bias_with_acc(void){
        // Тест в ускорении
    vec3_t gyro_bias_start = gyro_calibration_get_bias();
    injected_bias = (vec3_t){4,4,4};
    noise_level = 0.01f;
    move = 1.0f;
    // test_calibration_bias_make_move();

    float s_noise = move;
    for (int i = 0; i < CALIB_DURATION_CNT; i++) {
        vec3_t noise;
        // Добавляем шум
        noise.x = (float)(rand() % 100) / (100.0f/noise_level)+s_noise;
        noise.y = (float)(rand() % 100) / (100.0f/noise_level)+s_noise;
        noise.z = (float)(rand() % 100) / (100.0f/noise_level)+s_noise;
        s_noise += move;
        gyro_calibration_update(vec3_add(injected_bias, noise), false);
    }

    vec3_t gyro_bias_end = gyro_calibration_get_bias();

    TEST_ASSERT_FLOAT_WITHIN(noise_level, gyro_bias_start.x, gyro_bias_end.x);
    TEST_ASSERT_FLOAT_WITHIN(noise_level, gyro_bias_start.y, gyro_bias_end.y);
    TEST_ASSERT_FLOAT_WITHIN(noise_level, gyro_bias_start.z, gyro_bias_end.z);

    injected_bias = (vec3_t){5,5,5};
    noise_level = 0.01f;
    // Проверим в остановке
    RUN_TEST(test_calibration_bias_with_noise);
}

int main(void) {
    UNITY_BEGIN();

    // Тест с большими шумами
    injected_bias = (vec3_t){2,2,2};
    noise_level = 1.0f;

    RUN_TEST(test_calibration_bias_with_noise);


    // Тест с меньшими шумами
    injected_bias = (vec3_t){3,3,3};
    noise_level = 0.1f;

    RUN_TEST(test_calibration_bias_with_noise);


    // Тест в ускорении
    injected_bias = (vec3_t){4,4,4};
    noise_level = 0.2f;

    RUN_TEST(test_calibration_bias_with_acc);

    // Тест идеальный без шумов
    injected_bias = (vec3_t){6,6,6};
    noise_level = 0.01f;
    
    RUN_TEST(test_calibration_bias_with_noise);    // ideal value
    
    // RUN_TEST(test_calibration_fails_on_movement);
    return UNITY_END();
}


